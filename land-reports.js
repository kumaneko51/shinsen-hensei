...
function landReportsPage_() {
  ...
  const allRows = landRows_(), firstRows = allRows.filter(r => r.firstClear === true && r.result === '勝利');
  const rows = landScope === 'first' ? firstRows : allRows;
  const stats = landSummary_(firstRows), reference = landSummary_(allRows);
  ...
  '<p class="land-footnote">集計対象は「初攻略確認済み」の勝利のみ。レベルは戦闘開始時の値を使います。不明な凸数は0凸として数えません。シーズンをまたぐ比較には編成・戦法の違いがあります。</p>'+
  ...
  '<div class="land-toolbar"><h2>攻略記録</h2><label>表示 <select class="filter" id="land-scope"><option value="first" '+(landScope==='first'?'selected':'')+'>初攻略確認済み</option><option value="all" '+(landScope==='all'?'selected':'')+'>すべて（未確認・引分を含む）</option></select></label></div>'+
  (rows.length?'<div class="land-table-wrap"><table class="land-table"><thead><tr><th>編成・凸状況</th><th>開始時Lv</th><th>兵数</th><th>結果</th><th>初攻略</th><th>戦報</th></tr></thead><tbody>'+rows.map(r=>'<tr><td>'+r.team.split('・').map((name,i)=>'<div>'+name+' <small>'+ (r.breakthroughs[i]===null?'凸未確認':r.breakthroughs[i]+'凸')+'</small></div>').join('')+'</td><td>'+r.levels+'<small>部隊平均 '+landValue_(r.heroLevels.length===3?r.heroLevels.reduce((a,b)=>a+b,0)/3:null)+'</small></td><td>'+r.troops.toLocaleString()+'<small>減少 '+(r.troops?((r.dead+r.wounded)/r.troops*100).toFixed(1):'—')+'%</small></td><td>'+r.result+'</td><td>'+(r.firstClear===true?'確認済み':r.firstClear===false?'初攻略以外':'未確認')+'</td><td><button class="btn" data-modal="land-report:'+r.id+'">画像・詳細</button></td></tr>').join('')+'</tbody></table></div>':'<section class="empty-panel">'+(allRows.length?'初攻略確認済みの記録はありません。「すべて」に切り替えると既存の戦報を確認できます。':'この土地レベルのデータはまだありません。')+'</section>');
}
function landReportModal_(id) {
  if (!state.cloudUser || !state.family) return '';
  const r=LAND_REPORTS.find(x=>x.id===Number(id));
  if (!r) return '';
  return '<div class="backdrop"><section class="modal land-modal" role="dialog" aria-modal="true" aria-label="土地攻略事例の詳細"><div class="land-card-top"><h2>攻略記録 '+r.id+' · 土地Lv'+r.landLevel+'</h2><button class="btn" data-close-modal>閉じる</button></div><div class="land-detail"><div><h3>画像から確認した実績</h3><p>'+r.team+'</p><dl class="land-facts"><dt>初攻略</dt><dd>'+(r.firstClear===true?'確認済み':r.firstClear===false?'初攻略以外':'未確認')+'</dd><dt>シーズン</dt><dd>'+escape_(r.season)+'</dd><dt>凸状況</dt><dd>'+r.breakthroughs.map(n=>n===null?'未確認':n+'凸').join('・')+'</dd><dt>開始時Lv</dt><dd>'+r.levels+'</dd><dt>開始 → 残存兵数</dt><dd>'+r.troops.toLocaleString()+' → '+r.remaining.toLocaleString()+'</dd><dt>戦死／負傷</dt><dd>'+r.dead.toLocaleString()+'／'+r.wounded.toLocaleString()+'</dd><dt>士気</dt><dd>'+r.morale+'</dd><dt>守備軍</dt><dd>'+r.enemy+'隊 · Lv'+(r.enemyLevel ?? '未確認')+' · '+(r.enemyTroops===null?'兵数未確認':r.enemyTroops.toLocaleString()+'兵')+'</dd><dt>結果</dt><dd>'+r.result+'</dd></dl><h3>装備戦法（武将順）</h3>'+r.skills.map((s,i)=>'<p><strong>'+r.team.split('・')[i]+'</strong><br>'+s+'</p>').join('')+'<h3>解説・暫定評価</h3><span class="land-rating" data-rating="'+r.rating+'">'+r.rating+'</span><p class="land-explanation">'+r.note+'</p><p>出典：提供された戦報画像 '+r.id+'。シーズン・撮影日・戦法レベルは未確認です。</p></div><figure>'+(landImageUrl ? '<a href="'+landImageUrl+'" target="_blank" rel="noopener"><img src="'+landImageUrl+'" alt="事例'+r.id+'の土地Lv'+r.landLevel+'戦報"></a><figcaption>画像を押すと原寸で開きます</figcaption>' : '<p>'+(landImageStatus === 'error' ? '画像を取得できませんでした。詳細を開き直してください。' : '戦報画像を読み込んでいます…')+'</p>')+'</figure></div></section></div>';
}
