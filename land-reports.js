...
function landWinningAttempts_(rows) {
  const seen = new Set();
  return rows.flatMap(r => {
    ...
    if (!sequence.some(x=>x.result==='勝利')) return [];
...
function landReportsPage_() {
  ...
  return head+'<nav class="land-levels" aria-label="土地レベル">'+levels+'</nav><div class="land-toolbar"><div><h2>土地Lv'+landLevel+' の初攻略</h2><span>攻略成功 '+stats.count+'件 ／ 登録 '+allRows.length+'件</span></div><label>シーズン <select class="filter" id="land-season"><option value="all">すべて</option>'+seasons.map(v=>'<option value="'+escape_(v)+'" '+(landSeason===v?'selected':'')+'>'+escape_(v)+'</option>').join('')+'</select></label></div>'+
