...
test('九州争覇の新武将6人を選択できる',()=>{
  for(const [name,cost,skill] of expected){ const hero=master.heroes.find(x=>x.name===name); assert.ok(hero,`${name}が存在すること`); assert.equal(hero.cost,cost); assert.equal(hero.uniqueSkill,skill); assert.equal(hero.rarity,'S'); }
});
